...
class Wishart(Distribution):
    ...
    def pdf(self, x):
        ...
        N = len(x)
        if not (
            chk.has_shape([N, self._p, self._p])(x) and np.allclose(x, x.conj().transpose(0, 2, 1))
        ):
            raise ValueError("Parameter[x] must be hermitian symmetric.")

        if np.linalg.matrix_rank(self._V) < self._p:
            raise linalg.LinAlgError(
                "Wishart density is not defined when scale matrix V is singular."
            )
        ...
        num = np.float_power(dX, (self._n - self._p - 1) / 2) * np.exp(-trA / 2)
        den = (
            np.float_power(2, self._n * self._p / 2)
...
class Kent(Distribution):
    r"""
    `Kent <https://en.wikipedia.org/wiki/Kent_distribution>`_ distribution, also known as
    :math:`\text{FB}_{5}`, the 5-parameter Fisher-Bingham distribution.
    ...
    def angular_support(cls, k, beta, eps=1e-2):
        ...
        :math:`\text{FB}_{5}`, :py:meth:`~pypeline.util.math.stat.Kent.angular_support` returns
    ...
    def min_scale(cls, alpha, beta, eps=1e-2):
        ...
        For a given parameterization :math:`k, \beta, \gamma_{1}, \gamma_{2}, \gamma_{3}` of
        :math:`\text{FB}_{5}`, :py:meth:`~pypeline.util.math.stat.Kent.min_scale` returns the
