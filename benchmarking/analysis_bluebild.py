...
for ti in ProgressBar(time[timeslice]): 
    ...
    D, V = S_dp(G)  # D positive eigenvalues and V eigenvectors
    W = W.data      # beamforming matrix
...
I_lsq_eq_ss_interp = s2image.Image(f_interp_ss, pix_xyz_interp).data
...
I_lsq_eq_ps_interp = s2image.Image(f_interp_ps, pix_xyz_interp).data
...
ax[2].imshow(np.sum(I_lsq_eq_nufft.data, axis=0))
